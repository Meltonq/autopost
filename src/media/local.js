...
export function pickLocalImage({ rubric, imagesDir, usedStore, maxBytes }) {
  const used = usedStore.read();
  const key = rubric || "default";
  if (!used[key]) used[key] = [];

  const rubricDir = path.join(imagesDir, key);
  const baseDir = imagesDir;
  const files = listImages(rubricDir);
  const fallbackFiles = files.length ? files : listImages(baseDir);

  if (!fallbackFiles.length) {
    throw new Error(`No local images found in ${rubricDir} or ${baseDir}`);
  }
